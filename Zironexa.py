...
PLANES = {
    1: {
        "nombre": "Plan Free", "precio": 500, "ganancia_diaria": 20, "porcentaje": 4.0
    },
    2: {
        "nombre": "Plan Básico", "precio": 1500, "ganancia_diaria": 67, "porcentaje": 4.5
    },
    3: {
        "nombre": "Plan Intermedio", "precio": 5000, "ganancia_diaria": 250, "porcentaje": 5.0
    },
    4: {
        "nombre": "Plan Avanzado", "precio": 10000, "ganancia_diaria": 550, "porcentaje": 5.5
    },
    5: {
        "nombre": "Plan Profesional", "precio": 25000, "ganancia_diaria": 1500, "porcentaje": 6.0
    },
    6: {
        "nombre": "Plan Empresarial", "precio": 50000, "ganancia_diaria": 3250, "porcentaje": 6.5
    },
    7: {
        "nombre": "Plan Premium", "precio": 100000, "ganancia_diaria": 7000, "porcentaje": 7.0
    },
    8: {
        "nombre": "Plan VIP", "precio": 200000, "ganancia_diaria": 15000, "porcentaje": 7.5
    },
    9: {
        "nombre": "Plan Master", "precio": 400000, "ganancia_diaria": 32000, "porcentaje": 8.0
    },
    10: {
        "nombre": "Plan Zyronexa Elite", "precio": 1000000, "ganancia_diaria": 100000, "porcentaje": 10.0
    }
}
...
return jsonify({
    "success": False, "error": "El teléfono debe tener 8 dígitos"
}), 400
if len(contrasena) < 6:
return jsonify({
...
return jsonify({
    "success": True, "redirect": "/dashboard", "message": "Registro exitoso. Recibiste C$500 de bono"
})
...
return jsonify({
    "error": "Monto mínimo de retiro: C$360"
}), 400
...
return jsonify({
    "error": "Banco/Billetera destino requerido"
}), 400
...
datos_retiro = {
    "metodo": "tarjeta", "tarjeta": numero_tarjeta, "banco": banco_destino, "titular": nombre_titular
}
elif metodo == "billetera":
celular_billetera = data.get("celular", "").strip()
if not re.match(r'^\d{8}$', celular_billetera):
return jsonify({
    "error": "Celular de billetera inválido"
}), 400
datos_retiro = {
    "metodo": "billetera", "celular": celular_billetera, "billetera": banco_destino, "titular": nombre_titular
}
elif metodo == "cuenta":
...
datos_retiro = {
    "metodo": "cuenta", "cuenta": numero_cuenta, "banco": banco_destino, "titular": nombre_titular
} else :
return jsonify({
    "error": "Método de retiro no válido"
}), 400
...
conexion.close()
return jsonify({
    "error": "Solo puedes retirar saldo real. El bono no es retirable"
}), 400
...
conexion.close()
return jsonify({
    "error": "Solo 1 retiro por día permitido"
}), 400
...
return jsonify({
    "success": True, "message": f"Solicitud creada. Recibirás C$ {
        monto_neto
    } en 1-7 días hábiles"
})
except Exception as e:
...
mensaje = f"Plan mejorado a {
    PLANES[plan_valido]['nombre']}"
es_upgrade = True
else :
precio_a_pagar = precio_plan
mensaje = f"Plan activado: {
    PLANES[plan_valido]['nombre']}"
es_upgrade = False
...
conexion.close()
return jsonify({
    "error": "No tienes producto activo"
}), 400
...
conexion.close()
return jsonify({
    "error": "Ya cobraste hoy. Vuelve mañana"
}), 400
...
cursor.execute("INSERT INTO historial (telefono, tipo, monto, descripcion, estado) VALUES (%s, 'comision', %s, 'completado')", (PROPIETARIO_TELEFONO, comision_propietario, f'Comisión 10% de {
    usuario["telefono"]}'))
conexion.commit()
cursor.close()
conexion.close()
return jsonify({
    "success": True, "message": f"Ganaste C$ {
        ganancia_usuario
    }. Puedes cobrar todos los días"
})
...
ganancia_propietario = precio_a_pagar if es_upgrade else precio_plan
mensaje = f'Mejora a {
    PLANES[plan_id]["nombre"]}' if es_upgrade else f'Compra {
...
conexion.close()
return jsonify({
    "success": True, "message": "Retiro registrado. Haz el Payout manual en Stripe Dashboard"
})
...
if comprobante:
filename = secure_filename(f"retiro_ {
    id
}_ {
    datetime.now().timestamp()}.jpg")
...
return send_file(output, as_attachment = True, download_name = f'retiros_ {
...
conexion.close()
return jsonify({
    "success": True, "message": "Usuarios sincronizados"
})
